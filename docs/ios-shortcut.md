...
iOS Safari の共有シートからショートカットを起動し、Amazon の書誌情報を Obsidian Vault に Markdown として保存する手順です。
...
- iOS の Safari (動作確認済み。iPadOS での動作は未検証)
- iOS のショートカットアプリ
...
- Node.js (ビルド用、macOS 母艦で 1 度だけ実行)
...
`build/shortcut.js` が生成されます。中身は 1 ファイルにまとめられた JavaScript で、Safari の「Web ページで JavaScript を実行」アクションにそのまま貼り付けて使います。`config.ts` の値はビルド時にバンドル末尾へ埋め込まれるので、貼り付け後の手動編集は不要です。
